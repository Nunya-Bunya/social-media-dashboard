...
const AiCampaignGenerator = ({ tenantId, onCampaignGenerated, onClose }) => {
    const [step, setStep] = (0, react_1.useState)(1);
    const [loading, setLoading] = (0, react_1.useState)(false);
    const [error, setError] = (0, react_1.useState)(null);
    const [generatedPlan, setGeneratedPlan] = (0, react_1.useState)(null);
    ...
    const handleAdditionalGoalToggle = (goal) => {
        if (goal === formData.primaryGoal)
            return;
        setFormData(prev => ({
            ...prev,
            additionalGoals: prev.additionalGoals.includes(goal)
    ...
    const renderStep1 = () => (<div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          ...
          <input type="text" value={formData.clientName} onChange={(e) => handleInputChange('clientName', e.target.value)} className="mt-1 block w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-blue-500 focus:border-blue-500" placeholder="Enter client name"/>
        </div>
        
        <div>
          <label className="block text-sm font-medium text-gray-700">Business Type/Industry *</label>
          <input type="text" value={formData.businessType} onChange={(e) => handleInputChange('businessType', e.target.value)} className="mt-1 block w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-blue-500 focus:border-blue-500" placeholder="e.g., E-commerce, SaaS, Restaurant"/>
        </div>
      </div>

      <div>
        <label className="block text-sm font-medium text-gray-700">Target Audience *</label>
        <textarea value={formData.targetAudience} onChange={(e) => handleInputChange('targetAudience', e.target.value)} rows={3} className="mt-1 block w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-blue-500 focus:border-blue-500" placeholder="Describe your ideal customer (age, interests, pain points, etc.)"/>
      </div>

      <div>
        <label className="block text-sm font-medium text-gray-700">Timeline</label>
        <select value={formData.timeline} onChange={(e) => handleInputChange('timeline', e.target.value)} className="mt-1 block w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-blue-500 focus:border-blue-500">
    ...
    const renderStep2 = () => (<div className="space-y-6">
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {campaignGoals.map(goal => (<button key={goal.value} type="button" onClick={() => handleGoalToggle(goal.value)} className={`p-4 text-left border rounded-lg transition-colors ${formData.primaryGoal === goal.value
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {campaignGoals
            ...
            .map(goal => (<button key={goal.value} type="button" onClick={() => handleAdditionalGoalToggle(goal.value)} className={`p-4 text-left border rounded-lg transition-colors ${formData.additionalGoals.includes(goal.value)
                ? 'border-blue-500 bg-blue-50'
                : 'border-gray-200 hover:border-gray-300'}`}>
                <div className="font-medium text-gray-900">{goal.label}</div>
                <div className="text-sm text-gray-500 mt-1">{goal.description}</div>
      ...
      <div>
        ...
        <div className="space-y-3">
          {budgetRanges.map(budget => (<button key={budget.value} type="button" onClick={() => handleInputChange('budgetRange', budget.value)} className={`w-full p-4 text-left border rounded-lg transition-colors ${formData.budgetRange === budget.value
                ? 'border-blue-500 bg-blue-50'
                : 'border-gray-200 hover:border-gray-300'}`}>
      ...
      <div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
          {platforms.map(platform => (<button key={platform} type="button" onClick={() => handlePlatformToggle(platform)} className={`p-3 text-sm border rounded-lg transition-colors ${formData.preferredPlatforms.includes(platform)
      ...
      <div>
        ...
        <textarea value={formData.existingAssets} onChange={(e) => handleInputChange('existingAssets', e.target.value)} rows={3} className="mt-1 block w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-blue-500 focus:border-blue-500" placeholder="Describe any existing marketing materials, brand guidelines, or assets"/>
      </div>
    </div>);
    const renderGeneratedPlan = () => (<div className="space-y-6">
      <div className="text-center">
        <outline_1.CheckCircleIcon className="mx-auto h-12 w-12 text-green-500"/>
      ...
      {generatedPlan && (<div className="bg-gray-50 rounded-lg p-6 space-y-4">
          <div>
      ...
      <div className="flex space-x-3">
        <button onClick={handleUsePlan} className="flex-1 bg-blue-600 text-white px-4 py-2 rounded-md hover:bg-blue-700 transition-colors">
          Use This Plan
        </button>
        <button onClick={() => setStep(1)} className="flex-1 bg-gray-200 text-gray-700 px-4 py-2 rounded-md hover:bg-gray-300 transition-colors">
          Generate New Plan
        </button>
      </div>
    </div>);
      ...
      <div className="relative top-20 mx-auto p-5 border w-11/12 md:w-3/4 lg:w-1/2 shadow-lg rounded-md bg-white">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center space-x-2">
            <outline_1.SparklesIcon className="h-6 w-6 text-blue-600"/>
        ...
        <div className="flex items-center justify-center mb-8">
          <div className="flex items-center space-x-4">
            {[1, 2, 3].map((stepNumber) => (<div key={stepNumber} className="flex items-center">
                ...
                {stepNumber < 3 && (<div className={`w-16 h-0.5 ${step > stepNumber ? 'bg-blue-600' : 'bg-gray-200'}`}/>)}
              </div>))}
          </div>
        </div>

        
        {error && (<div className="mb-6 p-4 bg-red-50 border border-red-200 rounded-md">
            <div className="flex">
        ...
        {step < 3 && (<div className="flex justify-between mt-8">
            <button onClick={() => setStep(prev => Math.max(1, prev - 1))} disabled={step === 1} className="px-4 py-2 border border-gray-300 rounded-md text-gray-700 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed">
              Previous
            </button>
            
            {step === 1 ? (<button onClick={() => setStep(2)} className="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700">
                Next
              </button>) : (<button onClick={generateCampaign} disabled={loading} className="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed flex items-center space-x-2">
                {loading ? (<>
                    <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-white"></div>
                    <span>Generating...</span>
                  </>) : (<>
                    <outline_1.SparklesIcon className="h-4 w-4"/>
...
exports.default = AiCampaignGenerator;
//# sourceMappingURL=AiCampaignGenerator.js.map