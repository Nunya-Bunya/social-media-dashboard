...
export class CampaignAiService {
  ...
  async generateCampaignPlan(generateDto: GenerateCampaignDto, tenantId: string) {
  ...
  async generateCampaignAssets(campaignId: string, assetType: string, context: any, tenantId: string) {
  ...
  async generateCampaignSuggestions(clientId: string, brandId: string, tenantId: string) {
    // Get client and brand data for context
  ...
  async optimizeCampaignContent(campaignId: string, content: string, optimizationType: string, tenantId: string) {
    // Verify campaign exists and belongs to tenant
    const campaign = await this.prisma.campaign.findFirst({
      where: { id: campaignId, tenantId },
  ...
  private buildCampaignPrompt(generateDto: GenerateCampaignDto): string {
...
Return the response in this exact JSON format:
{
  ...
  private buildAssetPrompt(assetType: string, campaign: any, context: any): string {
...
Generate high-quality, engaging content that aligns with the campaign objectives and brand voice.`;
  ...
  private buildSuggestionPrompt(client: any, brand: any, performanceData: any): string {
...
- Success metrics`;
  ...
  private buildOptimizationPrompt(content: string, optimizationType: string, campaign: any): string {
...
- Optimize for the specific platform/channel`;
  ...
  private parseCampaignPlan(aiResponse: string, generateDto: GenerateCampaignDto): any {
    try {
  ...
  private calculateBudget(budgetRange: string): number {
    switch (budgetRange) {
  ...
  private analyzeCampaignPerformance(campaigns: any[]): any {
    ...
    const totalMetrics = campaigns.reduce((acc, campaign) => {
      if (campaign.analytics && campaign.analytics.length > 0) {
        campaign.analytics.forEach((analytics: any) => {
    ...
    return {
      ...
      topPerformers: campaigns
        ...
        .sort((a, b) => {
          const aRevenue = a.analytics.reduce((sum: number, analytics: any) => sum + (analytics.revenue || 0), 0);
          const bRevenue = b.analytics.reduce((sum: number, analytics: any) => sum + (analytics.revenue || 0), 0);
          return bRevenue - aRevenue;
        })
        .slice(0, 3)
        .map(c => ({ id: c.id, name: c.name, revenue: c.analytics.reduce((sum: number, analytics: any) => sum + (analytics.revenue || 0), 0) })),
  ...
  private parseSuggestions(aiResponse: string): any[] {
    try {
      ...
      // If parsing fails, create basic suggestions
      return [
        {
  ...
  private async saveGeneratedPlan(plan: any, generateDto: GenerateCampaignDto, tenantId: string): Promise<any> {
    // Create a new campaign record with the generated plan
    const campaign = await this.prisma.campaign.create({
      data: {
        name: plan.campaignName,
        description: plan.tagline,
        goal: generateDto.primaryGoal as any,
        type: this.mapGoalToType(generateDto.primaryGoal),
        status: 'DRAFT',
        brandId: generateDto.brandId,
        tenantId,
        createdBy: 'AI_SYSTEM', // Will be updated when user creates campaign
        content: plan,
        targetAudience: {
  ...
  private calculateEndDate(timeline: string): Date {
    const now = new Date();
