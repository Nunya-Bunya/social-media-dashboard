...
export class CampaignTemplatesService {
  ...
  async createTemplate(templateDto: CampaignTemplateDto) {
  ...
  async updateTemplate(id: string, templateDto: Partial<CampaignTemplateDto>) {
  ...
  async getTemplateSuggestions(goal: CampaignGoal, type?: CampaignType, budget?: number) {
    // Get templates that match the goal and type
    const where: any = { goal, isActive: true };
    ...
    // Score templates based on relevance and budget compatibility
  ...
  private calculateTemplateScore(template: any, goal: CampaignGoal, type?: CampaignType, budget?: number): number {
    let score = 0;

    // Base score for goal match
    if (template.goal === goal) score += 10;

    // Bonus for type match
    if (type && template.type === type) score += 5;

    // Bonus for high usage (proven effectiveness)
