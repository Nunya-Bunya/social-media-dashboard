...
// Test scraper endpoint (bypasses rate limiting)
  ...
  try {
    ...
    const job = await prisma.scrapeJob.create({
      data: {
        ...
        status: 'PENDING',
        progress: 0,
        createdBy: 'cmerrwet60002lu56r2kv69mp', // Test user
        tenantId: 'cmerrwet40000lu567mne69ly' // Test tenant
      }
    });

    // Run scraper service directly
    const scraperService = new ScraperService();
    const leads = await scraperService.runScrapeJob(job.id);
...
app.listen(PORT, async () => {
  ...
  console.log(`🔍 Scraper endpoints at http://localhost:${PORT}/api/scraper`);
  console.log(`📊 Lead management at http://localhost:${PORT}/api/leads`);
